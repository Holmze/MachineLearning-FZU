...
fig = plt.figure(1)
x1, y1 = make_circles(n_samples=300,factor=0.05, noise=0.1)     # 生成二维数组
...
## 进行预测
x2 = (random.random()-0.5)*2
y2 = (random.random()-0.5)*2
print(x2)
print(y2)
X_sample = np.array([[x2, y2]])
neighbors = clf.kneighbors([[x2,y2]],return_distance=False)
#print(neighbors)
##输出配置
plt.subplot(121)     # 一行两列 当前为1(第一行第一列)
#plt.title('make_circlesz 1')
...
for i in neighbors[0]:
    plt.plot([x1[i][0], X_sample[0][0]], [x1[i][1], X_sample[0][1]], 'r.-')
